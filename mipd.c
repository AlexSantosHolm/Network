...
static void handle_raw_socket(int fd, int debug) {
    ...
    if (debug) {
        printf("Revieved %d bytes on raw socket", rc);
    }
}
...
static void handle_unix_socket(int fd, int unix_sock, int debug) {
    ...
    if (fd == unix_sock) {
        ...
        if (debug) {
            printf("Recieved from client %d: %d bytes\n", fd, rc);
...
int main(int argc, char *argv[]) {
    ...
    while ((opt = getopt(argc, argv, "hd")) != -1) {
        switch (opt) {
            ...
            break;
            default:
            printf("Usage: %s [-h] [-d] <socket_upper> <MIP_adress>\n", argv[0]);
            exit(EXIT_FAILURE);
        }
    }

    if (optind + 2 != argc) {
        printf("Usage: %s [-h] [-d] <socket_upper> <MIP_adrdress>\n", argv[0]);
        exit(EXIT_FAILURE);
    }
