...
static struct pending_packet pending_packets[MAX_PENDING];

static void init_pending_packet(void) {
	memset(pending_packets, 0, sizeof(pending_packets));
}

static void queue_pending_packet(uint8_t dst_addr, uint8_t src_addr, const char* payload, int payload_len, int client_fd) {
	for (int i = 0; i < MAX_PENDING; i++) {
		if (!pending_packets[i].valid) {
			pending_packets[i].dest_addr = dst_addr;
			pending_packets[i].src_addr = src_addr;
			memcpy(pending_packets[i].payload, payload, payload_len);
			pending_packets[i].payload_len = payload_len;			
			pending_packets[i].client_fd = client_fd;
			pending_packets[i].timestamp = time(NULL);
			pending_packets[i].valid = 1;
			return;
		}
	}
}
...
static void handle_raw_socket(int fd, uint8_t my_mip_addr, int debug) {
    ...
    switch (mip_hdr->msg_type) {
	    ...
	    send_pending_packets(fd, mip_hdr->src_addr, debug);
...
static void handle_unix_socket(int fd, int unix_sock, int raw_sock, 
    ...
    if (fd == unix_sock) {
        ...
        if (dest_addr == my_mip_addr) {
            ...
            if (send_mip_packet(raw_sock, dest_addr, my_mip_addr, 
                               message, msg_len, debug) == -2) {
		    queue_pending_packet(dest_addr, my_mip_addr, message, message_len, fd)
                if (debug) {
                    printf("Packet queued, waiting for ARP resolution\n");
                }
            }
        }
    }
...
static void send_pending_packets(int raw_sock, uint8_t dst_addr, int debug) {
	for (int i = 0; i < MAX_PENDING; i++) {
		if (pending_packets[i].valid && pending_packets[i].dest_addr == dst_addr) {
			if (debug) {
				printf("Sending queued packet to MIP %d\n", dst_addr);
			}
			send_mip_packet(raw_sock, pending_packets[i].dest_address, pending_packets[i].src_addr, pending_packets[i].payload, pending_packets[i].payload_len, debug);
			pending_packets[i].valid = 0;
		}
	}
}
